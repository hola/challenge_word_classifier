...
(function (exports, buffer, dataSize, binSize, readInt32LE) {
    ...
    function splitSamples(word, minWordSize) {
        var samples = [],
            i = 0;
        if (word.slice(-2) == '\'s') word = word.slice(0, -2);
        if (word.length < minWordSize) return [];
		word = '`' + word + '`';
    ...
    var initTxt = function (data) {
        var contents = ArchUtils.bz2.decode(data);
        var lines = contents.split('\n');
        var line = null;
        var prevLine = '';
        var readSamples = true;
        var rejectedKey = null;
        ...
        for (var i = 0; i < lines.length; i++) {
            ...
            if (line.length == 0) {
                ...
                if (readSamples) {
                    ...
                    if (rejectedKey == null) {
                        rejectedKey = sample;
                        rejectedPairs[rejectedKey] = [];
                    } else {
                        rejectedPairs[rejectedKey].push(sample);
                    }
                }
                prevLine = sample;
            }
    ...
    exports.test = function (word) {
        ...
        var samples1 = splitSamples(word.substr(7), sampleSize);
        var samples2 = splitSamples(word.substr(4), sampleSize + 1);
        var sample = null;

        for (var i = 0; i < samples1.length; i++) {
            if (samples1[i] in rejectedSamples) return false;
        }

        for (var j = 0; j < samples2.length; j++) {
            var x = samples2[j];
            for (var k = 0; k < samples2.length; k++) {
                var y = samples2[k];
                if (x >= y) continue;
                var rejected = rejectedPairs[x];
                if (rejected && rejected.indexOf(y) !== -1) return false;
            }
        }

        if (bloom) {
            var seq = 'erinreanaronraroenriun',
                prep = (word.slice(-2) == '\'s') ? word.slice(0, -2) : word,
                i = 0,
                s; 
            for (; i < seq.length ; s = seq.slice(i, i + 2), prep = prep.split(s).join(String.fromCharCode((i / 2))), i += 2);
            if (!bloom.test(prep.substr(0, 6))) return false;
        } 
