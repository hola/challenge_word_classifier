// usage:
// node mkdata-tree-as-bitmap.js <words.lc.txt |gzip --best >data.gz

'use strict';

const depth = 14;
const bitmap_length = 16;
var data = '';
...
process.stdin.on('end', function() {
    let buf = init_buffer_tree_as_bitmap(depth);
    data.split("\n").forEach( function(element, index, array) { fill_buffer_tree_as_bitmap(buf,element) });
    process.stdout.write(buf);
});

String.prototype.char_code = function(pos) { let c = this.charCodeAt(pos)-96; return(c<0?0:c) };

function init_buffer_tree_as_bitmap(depth) {
    return Buffer.alloc(27*27*bitmap_length*depth, 0, "binary")
}

function fill_buffer_tree_as_bitmap(buf,word) {
    if (!RegExp("^[a-z,']+$").test(word)) {return}

    let eff_depth = (word.length<depth)?word.length:depth;
    let md5_truncated = rstr_md5(word).charCodeAt(0) & 0x7F;
    let b_md5_offset = Math.floor(md5_truncated/8);
    let bmp_md5 = 1<<(md5_truncated%8);

    for(let pos=0; pos<(eff_depth-1); pos++) {
        let b_md5_pos = pos*27*27*bitmap_length + word.char_code(pos)*27*bitmap_length + word.char_code(pos+1)*bitmap_length + b_md5_offset;
        buf[b_md5_pos] |= bmp_md5;
    }
}

// stolen from http://pajhome.org.uk/crypt/md5
