...
public class BloomFilter<E> implements Serializable {
    ...
    public static interface CalcHash {
        /**
        * @param data specifies input data.
        * @param hashes number of hashes/int's to produce.
        * @return array of int-sized hashes
        */
        int[] createHashes(byte[] data, int hashes);

        int adjustToBitSetSize(int hash, int bitSetSize);
    }

    public static CalcHash calcHash;
    ...
    public static int[] createHashes(byte[] data, int hashes) {
        ...
        if (calcHash != null) return calcHash.createHashes(data, hashes);
    ...
    public void add(byte[] bytes) {
       ...
       for (int hash : hashes)
           bitset.set(adjustHashToBitSetSize(hash), true);
       numberOfAddedElements ++;
    }

    private int adjustHashToBitSetSize(int hash) {
        if (calcHash != null) return calcHash.adjustToBitSetSize(hash, bitSetSize);
        int n = Integer.remainderUnsigned(hash, bitSetSize);
        return n;
    }
    ...
    public boolean contains(byte[] bytes) {
        int[] hashes = createHashes(bytes, k);
        for (int hash : hashes) {
            if (!bitset.get(adjustHashToBitSetSize(hash))) {
                return false;
            }
        }
        return true;
