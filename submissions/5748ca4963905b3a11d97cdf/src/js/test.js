...
function main(target, testcases){
    ...
    if (fs.lstatSync(testcases).isFile()){
		var res=[0,0]
    	/*
    	//не подходит, т.к. работает асинхронно
		...
		*/
		//var lines = fs.readFileSync(testcases).toString().split('\r\n').forEach(function (line) {
		var lines = fs.readFileSync(testcases).toString().split('\n').forEach(function (line) {
				...
				if(line.length>0){
					if(/\s/.test(line)){
						...
						try{
							var t=test(context, line)
							if(!t)
								console.log('bad word',line)
							//console.log(line,t)
							res[t?1:0]++
						}catch(err){					
							console.log('Error at word',line.length);
						}
					}
				}
    	});
		console.log('bad=',res[0],'good=',res[1]);
