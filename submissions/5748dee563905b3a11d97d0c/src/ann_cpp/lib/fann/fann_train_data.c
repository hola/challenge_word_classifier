...
/*
 * deallocate the train data structure. use in conjuction with fann_create_train_noalloc()
 */
FANN_EXTERNAL void FANN_API fann_destroy_train_noalloc(struct fann_train_data *data)
{
	if(data == NULL)
		return;
	fann_safe_free(data->input);
	...
	for(i = 0; i != data->num_data; i++)
	{
		fann_train(ann, data->input[i], data->output[i]);
                /*if (i % (100 * 1000) == 0) {
                    ...
                    udata = (ann_user_data*)(ann->user_data);
                    score = ann_test(ann, *(udata->test_datasets));
 ...
 * Internal train function 
 */
float fann_train_epoch_incremental_test(struct fann *ann, struct fann_train_data *data)
{
	unsigned int i;
        static thread_local float best_MSE = -1;
	...
	for(i = 0; i != data->num_data; i++)
	{
		fann_train(ann, data->input[i], data->output[i]);
                ...
                if (i % 100000 == 0) {
                    //if (fann_get_MSE(ann) < best_MSE || best_MSE == -1)
                    {
                        best_MSE = fann_get_MSE(ann);
                        if (ann->test_callback != NULL) {
                            (*ann->test_callback)(ann);
                        }
                    }
                }
	}
...
FANN_EXTERNAL float FANN_API fann_train_epoch(struct fann *ann, struct fann_train_data *data)
{
	...
	case FANN_TRAIN_INCREMENTAL:
        ...
        case FANN_TRAIN_INCREMENTAL_TEST:
                return fann_train_epoch_incremental_test(ann, data);
	}
	return 0;
}
 ...
 * Creates an empty set of training data
 */
FANN_EXTERNAL struct fann_train_data * FANN_API fann_create_train_noalloc(unsigned int num_data, unsigned int num_input, unsigned int num_output)
{
	fann_type *data_input, *data_output;
	unsigned int i;
	struct fann_train_data *data =
...
#define SCALE_SET_PARAM( where )																		\
	...
	for( cur_neuron = 0; cur_neuron < ann->num_##where##put; cur_neuron++ )								\
		ann->scale_mean_##where[ cur_neuron ] /= data->num_data;									\
	...
	for( cur_neuron = 0; cur_neuron < ann->num_##where##put; cur_neuron++ )								\
		ann->scale_deviation_##where[ cur_neuron ] =													\
			sqrt( ann->scale_deviation_##where[ cur_neuron ] / data->num_data ); 			\
