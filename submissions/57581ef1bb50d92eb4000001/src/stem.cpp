...
static void step1ab(struct stemmer * z)
{
    ...
    if (b[z->k] == 's')
    {
        if (z->k>3 && ends(z, "\04" "sses")) z->k -= 2; else
        if (z->k>2 && ends(z, "\03" "ies")) setto(z, "\01" "i"); else
        if (b[z->k - 1] != 's') z->k--;
    }
...
int stem_int(char *str, size_t len)
{
    ...
    stemmer *z = &st;
    len -= 1;
    if (len <= 1) return (int)len+1; /*-DEPARTURE-*/
    z->b = str; z->k = (int)len; /* copy the parameters into z */
    ...
    assert(len+1 >= (size_t)z->k+1);
    return z->k+1;
}

int stem_int(char *str)
{
    return stem_int(str, strlen(str));
}
